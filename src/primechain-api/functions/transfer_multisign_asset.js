...
exports.transfer_multisign_asset = (senders_address, receivers_address, asset_name, asset_quantity, senders_private_key) => {
    return new Promise(async function (resolve, reject) {
        ...
        let newaddress = await bcSdk.createRawSendFrom({
            ...
            bcSdk.listUnspent({
                ...
                var redeem_script = res.response[0].redeemScript;
                var script_pub_key = res.response[0].scriptPubKey;
                var tx_id = res.response[0].txid;
                var vout = res.response[0].vout;
                bcSdk.signRawMultiSignTransaction({
                    hexstring: newaddress.response["hex"],
                    prevtexts: [{ "txid": tx_id, "vout": vout, "redeemScript": redeem_script, "scriptPubKey": script_pub_key }],
                    private_key: [senders_private_key[0]]
                }).then((res) => {
                    bcSdk.signRawMultiSignTransaction({
                        hexstring: res.response["hex"],
                        prevtexts: [{ "txid": tx_id, "vout": vout, "redeemScript": redeem_script, "scriptPubKey": script_pub_key }],
                        private_key: [senders_private_key[1]]
                    }).then((res) => {
