$('#BGIssue').off('click').on('click', function (e) {
    e.preventDefault();
    var formBody = {};
    var elem = document.getElementsByClassName('academic_form')[0].elements;
    ...
    $.ajax({
        type: "POST",
        url: '/plugins/dave/academic/issue',
        ...
        success: function (res) {
            if (res && res["success"]) {
                                                    ...
                                                    if (res["aes_iv"]) {
                                                        ...
                                                        if (res["trade_channel_name"]) {
                                                            ...
                                                            $('#td7').html(`<button class="btn btn-success m-2"
                                                                ...
                                                                data-clipboard-text="${window.location.origin}/plugins/dave/academic/verification?txid_data=${res['tx_id_enc_data']}&txid_signature=${res['tx_id_signature']}&password=${res['aes_password']}&iv=${res['aes_iv']}&trade_channel_name=${res['trade_channel_name']}">
                                                                Copy to clipboard
                                                            </button>`);
                ...
                alert(res['message']);
                window.location.href = '/plugins/dave/academic/issue'
            }
        },
        data: formBody,
    });
...
$('#retrieveBGClick').off('click').on('click', function (e) {
    ...
    if (txid_data && txid_signature && password && iv && trade_channel_name) {
        ...
        $.ajax({
            type: "POST",
            url: '/plugins/dave/academic/retrieve',
            ...
            success: function (res) {
                if (res && res["success"]) {
                    ...
                    alert(res['message'])
                    window.location.href = '/plugins/dave/academic/retrieve';
                }
            },
            data: formData,
        });
