    ...
    try {
      ...
      if (existingUser) {
        ...
        if (user) {
            ...
            message: req.flash(
              "error_msg",
              "You cannot use a third party login for this email address. Please enter the password you had set when creating this Primechain account."
            ),
          });
        } else {
          const { primechain_address } = await APICall.httpPostMethod(
            "create_entity",
            {
          ...
          if (primechain_address) {
            ...
            await create(newUser.email, ip, browser);
              ...
              message: req.flash(
                "error_msg",
                "Unable to set blockchain address, please check configuaration"
              ),
            });
          }
        }
        ...
        message: req.flash("error_msg", "Unable to sign in using google"),
      });
    }
  }
);
