...
module.exports = (tx_id_enc_data, tx_id_signature, aes_password, aes_iv, stream_name) => {
    return new Promise(async (resolve, reject) => {
        try {
            const validationErrors = [];

            if (!validator.isLength(tx_id_enc_data)) validationErrors.push({ msg: 'Transaction Id of the encrypted data is missing' })
            ...
            const response = await APICall.httpPostMethod('decrypt_download_data', {
                tx_id_enc_data,
                tx_id_signature,
                aes_password,
                aes_iv,
